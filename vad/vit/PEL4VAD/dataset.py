...
class PistachioDataset(data.Dataset):
    def __init__(self, cfg, transform=None, test_mode=False):
        ...
        self.abnormal_dict = {
                'normal':0,
                'theft': 1,
                'fighting_and_physical_conflict': 2,
                'weapons_incident': 3,
                'robbery': 4,
                'animal_abuse': 5,
                'slip_and_fall_accident': 6,
                'sudden_illness_and_seizure': 7,
                'sudden_illness_seizure': 7,
                'fire': 8,
                'vandalism': 9,
                'explosion': 10,
                'falling_object_and_collapse': 11,
                'animal_attack_or_fight': 12,
                'wild_large_animal_intrusion': 13,
                'pushing_conflict': 14,
                'extreme_weather_events': 15,
                'person_drowning': 16,
                'animal_predation': 17,
                'animal_fight': 18,
                'animal_fall_and_injury': 19,
                'natural_disasters': 20,
                'ground_collapse': 21,
                'landslide': 22,
                'medical_emergency': 23,
                'safety_violations': 24,
                'equipment_breakdown': 25,
                'construction_accident': 26,
                'traffic_accident': 27,
                'structural_failure': 28,
                'leakage': 29,
    ...
    def __getitem__(self, index):
        ...
        path_str = self.list[index].strip('\n')
        ...
        if not self.test_mode:
            ...
            if len(path_parts) >= 4 and path_parts[-4] == 'anomaly':
                video_ano = path_parts[-2] 
                label = 1.0
                if video_ano in self.abnormal_dict:
                    ano_idx = self.abnormal_dict[video_ano]
                else:
                    print(f"Warning: Class '{video_ano}' not in dict. Path: {path_str}")
                    ano_idx = 0 
                    
            elif len(path_parts) >= 3 and path_parts[-3] == 'normal':
                video_ano = 'normal'
                label = 0.0
                ano_idx = 0
            else:
            ...
            if len(path_parts) >= 5 and path_parts[-5] == 'anomaly':
                video_ano = path_parts[-4] 
                label = 1.0
                ano_idx = self.abnormal_dict.get(video_ano, 0)
