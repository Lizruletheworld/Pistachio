...
def test(dataloader, model, args, device):
    ...
    with torch.no_grad():
        ...
        if len(pred) == len(gt) * 16:
            print("GT and Pred mismatch (1:16). Repeating GT labels...")
            gt_for_eval = np.repeat(gt, 16) # (131680,)
        else:
            gt_for_eval = gt
            
        print(f"DEBUG: Using GT shape for eval: {gt_for_eval.shape}")


        fpr, tpr, threshold = roc_curve(list(gt_for_eval), pred)

        rec_auc = auc(fpr, tpr)

        precision, recall, th = precision_recall_curve(list(gt_for_eval), pred)
