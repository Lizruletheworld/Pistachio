    ...
    def __getitem__(self, idx):
        ...
        if self.old_data!=None:
            if k in self.old_data:
                return (idx, k, k ,k)
        video_path = resolve_pistachio_video_path(self.video_root, k)
...
if __name__ == "__main__":
    ...
    if args.video_root is None:
        if 'UCF_Crime' in video_summary_json:
            raise ValueError("Set --video_root for UCF-Crime.")
        elif 'XD' in video_summary_json:
            raise ValueError("Set --video_root for XD-Violence.")
        elif 'MSAD' in video_summary_json:
            raise ValueError("Set --video_root for MSAD.")
        elif 'Pistachio' in video_summary_json:
            video_root = str(pistachio_video_test_root())
