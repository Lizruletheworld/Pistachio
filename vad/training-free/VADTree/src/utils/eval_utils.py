...
Pistachio_classes = ['Normal', 'animal_abuse', 'animal_attack_or_fight', 'animal_fall_and_injury', 'animal_fight', 'animal_predation', 'avalanche', 'construction_accident', 'equipment_breakdown', 'explosion', 'extreme_weather_events', 'falling_object_and_collapse', 'fighting_and_physical_conflict', 'fire', 'ground_collapse', 'infrastructure_failure', 'landslide', 'leakage', 'medical_emergency', 'natural_disasters', 'person_drowning', 'pushing_conflict', 'robbery', 'safety_violations', 'slip_and_fall_accident', 'structural_failure', 'sudden_illness_and_seizure', 'theft', 'traffic_accident', 'vandalism', 'weapons_incident', 'wild_large_animal_intrusion']
...
def initialize_score_dicts(scores_json):
    ...
    elif 'PISTACHIO' in scores_json.upper():
        dataset_classes = Pistachio_classes
...
def get_video_category(scores_json, video_name):
    ...
    try:
        if 'UCF' in scores_json.upper():
            ...
            # 动态建立映射 (只在第一次运行时执行)
            global PISTACHIO_MAP
            if not PISTACHIO_MAP:
                # 注意：这里需要能访问到 annotation 文件的路径
                # 简单做法是根据 scores_json 推断路径，或者写死
                anno_path = scores_json.replace('result', 'dataset_info').split('Pistachio')[0] + 'Pistachio/annotations/Temporal_Anomaly_Annotation_for_Testing_Videos.txt'
                # 如果路径不对，请手动指定一个能找到该 txt 的路径
                try:
                    with open(anno_path, 'r') as f:
                        for line in f:
                            p = line.strip().split()
                            if len(p) >= 2:
                                # 建立 000000263811_all -> animal_abuse 的映射
                                # 移除 .mp4 后缀进行匹配
                                v_key = Path(p[0]).stem 
                                PISTACHIO_MAP[v_key] = p[1]
                except:
                    pass

            # 匹配类别
            v_stem = Path(video_name).stem
            if v_stem in PISTACHIO_MAP:
                return [PISTACHIO_MAP[v_stem]]
