...
from pistachio_paths import pistachio_video_test_root, resolve_pistachio_video_path
...
def main(cfg, args):
    ...
    elif 'Pistachio' in annotationfile_path:
        out_path = f'{base_dir}/result/Pistachio_test/EGEBD_{model_name}_split_out_th{threshold}/'
    else:
    ...
    # 1. 修改读取逻辑：不再只取 basename，而是保留原始路径
    video_list_ = [x.strip().split()[0] for x in open(annotationfile_path)]

    # 2. 排序逻辑可以保持（或者简化）
    video_list_ = sorted(video_list_, key=lambda x: ("ormal" in x or "label_A" in x, x))

    # 3. 核心修改：判断 txt 里是绝对路径还是文件名
    for path in video_list_:
        name = os.path.basename(path) # 用于作为字典的 key (比如 000000263811_all.mp4)
        
        if os.path.isabs(path) or "/" in path:
            # 如果是路径，直接用
            fnm_path_dict[name] = path
        else:
            # 如果只是文件名，才进行拼接
            fnm_path_dict[name] = os.path.join(video_dir, name + '.mp4')
