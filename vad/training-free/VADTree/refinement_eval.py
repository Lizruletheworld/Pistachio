...
def parse_args():
    ...
    if args.video_root is None:
        if 'UCF' in args.scores_json:
            ...
            if args.video_root is None: raise ValueError("Set --video_root for UCF-Crime.")
            ...
            if args.video_root is None: raise ValueError("Set --video_root for XD-Violence.")
            ...
            if args.video_root is None: raise ValueError("Set --video_root for MSAD.")
        elif 'Pistachio' in args.scores_json:
            args.normal_label = 0
            args.video_fps = 16.0 # MSAD数据集视频帧率不统一，自动获取
            args.annotationfile_path=f'{vadtree_path}/dataset_info/Pistachio/annotations/pistachio_anomaly_test.txt'
            args.temporal_annotation_file=f'{vadtree_path}/dataset_info/Pistachio/annotations/Temporal_Anomaly_Annotation_for_Testing_Videos.txt'
            if args.video_root is None: args.video_root = str(pistachio_video_test_root())
...
def main(
    ...
    args.vadtree_path = os.path.join(os.path.dirname(__file__), "result")
    ...
    if 'Pistachio' in args.scores_json: # load video info for fps
            with open(f'{args.vadtree_path}/Pistachio_test/EGEBD_x2x3x4_r50_eff_split_out_th0.5_peak_dfs_kmeans_1_0.4'
                      '/dfs_coarse_scenes.json') as f:
                pistachio_video_info = json.load(f)
